...
function [] = run_prepro(WhichProject,WhichSessScan,subject,smoothing,discard,slicetime,despike,detr,intnorm,bandpass,demean)
    ...
    switch cfg.WhichProject
        ...
        case 'COBRE'
            run('COBRE_config.m');
        case 'Beijing_Zang'
            run('Beijing_Zang_config.m');
        case 'COBRE_HCTSA'
            run('COBRE_HCTSA_config.m');
        case 'UCLA_HCTSA'
            run('UCLA_HCTSA_config.m');
        case 'NAMIC_HCTSA'
            run('NAMIC_HCTSA_config.m');
        case 'conc_TMS_FMRI'
            run('conc_TMS_FMRI_config.m');
        case 'Activebrains_pre'
            run('Activebrains_pre_config.m');
        case 'TBS_fMRI'
            run('TBS_fMRI_config.m');
        case 'EAT'
            run('EAT_config.m');
        case 'OCDPG_DCM'
            run('OCDPG_DCM_config.m');
    end
